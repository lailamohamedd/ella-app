...
<div class="product-details mt-16">
    ...
    <v-container fluid>
        <v-row>
            <v-col cols="7">
                <img :src="tab ? tab : singleProduct.thumbnail" v-if="!loading" height="500" class="w-100" alt="product image">
                <v-skeleton-loader v-if="loading" type="image, image, image"></v-skeleton-loader>
                <v-tabs center-active height="120" class="mt-10" v-model="tab">
            ...
            <v-col cols="5" class="pt-0 pl-6">
                ...
                <v-card elevation="0">
                    ...
                    <div class="rating-parent d-flex align-center" style="gap: 10px;">
                        <v-rating v-model="singleProduct.rating" half-increments readonly color="yellow-darken-2" size="x-small" density="small"></v-rating>
                        <span class="mt-1" style="font-size: 13px; color: #454545;">Stock: {{ singleProduct.stock }}</span>
                    </div>
                    ...
                    <v-card-actions class="mt-7 w-100 px-0">
                        <v-btn variant="outlined" class="w-75 text-white" density="compact" height="50" @click="addToCart(singleProduct)" :loading="btnLoading" style="text-transform: none; border-radius: 30px; background-color: #454545;">Add To Cart</v-btn>
                    </v-card-actions>
                </v-card>
            </v-col>
        </v-row>
