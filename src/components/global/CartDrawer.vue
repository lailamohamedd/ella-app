<template>
    <div class="drawer">
        ...
        temporary>
            ...
            <v-card class="pa-0 mt-5 items-card" v-if="cartItems.length" elevation="0" style="overflow-y: auto;" max-height="395">
                    ...
                    <v-row v-for="item in cartItems" :key="item.id" class="align-center">
                        ...
                        <v-col cols="9">
                            <div class="d-flex align-center">
                                <div>
                                    <v-card-title class="px-0" style="white-space: pre-wrap; font-size: 13px; line-height: 1.2;">
                                        {{ item.title }} Sample - {{ item.category }}
                                    </v-card-title>
                                    <v-card-text class="px-0 pb-0" style="white-space: pre-wrap; font-size: 12px; line-height: 1.2;">Category: {{ item.category }}</v-card-text>
                                    ...
                                    <div class="item-footer d-flex justify-lg-space-between align-center">
                                        <div class="counter px-1" style="border-radius: 30px; border: 1px solid #454545; width: fit-content;">
                                            ...
                                            <input type="number" class="text-center py-1" style="border: none; outline: none; width: 35px; font-size: 14px;" min="1" v-model="item.quantity" />
            ...
            <v-card class="pa-0 mt-5" elevation="0" v-if="cartItems.length">
                <v-card-actions class="flex-column justify-center align-center" style="gap: 15px;">
                    <v-btn style="text-transform: none; border-radius: 30px; border-color: #f9f9f9;" color="blue" variant="outlined" density="compact" height="45" @click="toCheckout" class="w-100 bg-info">Check Out</v-btn>
                    <v-btn style="text-transform: none; border-radius: 30px; border-color: #f9f9f9;" color="blue" variant="outlined" density="compact" height="45" @click="$router.push({name: 'cart_page'})" class="w-100">View Cart</v-btn>
                </v-card-actions>
            </v-card>
        </v-navigation-drawer>
    </div>
