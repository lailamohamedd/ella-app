<template>
    <div class="drawer">
        <v-navigation-drawer class="pr-1 cart-drawer" v-model="drawer" width="370" location="right"
        temporary>
            <v-card class="px-0" elevation="0">
                <v-card-title class="pl-0 pr-2 d-flex justify-lg-space-between align-center w-100" style="font-size: 17px; font-weight: bold;">
                    Shopping Cart
                    <v-icon @click="drawer = false">mdi-close</v-icon>
                </v-card-title>

                <!-- <v-card-text class="px-0">{{ cartItems.length }} Items</v-card-text> -->
                <v-card-text v-if="!cartItems.length" class="px-0">Free shipping for all orders over $10000.00</v-card-text>
                <v-card-text v-if="!cartItems.length" class="px-0 text-center">Your cart is empty</v-card-text>
                <div class="bar-parent mt-4 position-relative mr-2" v-if="cartItems.length">
                    <svg width="30" fill="#f44336" :style="`bottom: 50%; z-index: 1; left: calc((${
                    ...
                    <v-progress-linear color="red" striped height="10px" 
                    :model-value="parseInt((calcTotalPrice / 10000) * 100) <= 100 ? parseInt((calcTotalPrice / 10000) * 100) : 100"></v-progress-linear>
                </div>
                <v-card-text v-if="cartItems.length && 10000 - calcTotalPrice > 0" class="px-0 pt-2">Only ${{ 10000 - calcTotalPrice }}.00 away from Free Shipping</v-card-text>
                ...
                <v-card-actions v-if="!cartItems.length">
                    <v-btn style="text-transform: none; border-radius: 30px; border-color: #f9f9f9;" class="w-100" variant="outlined" density="compact" @click="drawer = false" height="45">Continue Shopping</v-btn>
                </v-card-actions>
            </v-card>
            <v-card class="pa-0 mt-5 items-card" v-if="cartItems.length" elevation="0" style="overflow-y: auto;" max-height="240">
                <v-container class="px-1">
                    <v-row v-for="item in cartItems" :key="item.id" class="align-center mb-4">
                        ...
                        <v-col cols="7">
                            <v-card-title style="white-space: pre-wrap; font-size: 14px; line-height: 1.2;">
                            ...
                            <div class="item-footer d-flex justify-lg-space-between align-center">
                                <div class="counter px-1" style="border-radius: 30px; border: 1px solid #454545; width: fit-content;">
                                    ...
                                    <input type="number" class="text-center py-3" style="border: none; outline: none; width: 35px; font-size: 14px;" min="1" v-model="item.quantity" />
                                    <v-icon size="22" @click="item.quantity++">mdi-plus</v-icon>
                                </div>
                                <v-icon siz="22" @click="deleteItem(item.id)">mdi-close</v-icon>
                            </div>
                        </v-col>
                    </v-row>
                </v-container>
            ...
            <v-card class="pa-0 mt-5" elevation="0" v-if="cartItems.length">
                <v-card-actions class="flex-column justify-center align-center" style="gap: 15px;">
                    <v-btn style="text-transform: none; border-radius: 30px; border-color: #f9f9f9;" color="blue" variant="outlined" density="compact" height="45" class="w-100 bg-info">Check Out</v-btn>
                    <v-btn style="text-transform: none; border-radius: 30px; border-color: #f9f9f9;" color="blue" variant="outlined" density="compact" height="45" class="w-100">View Cart</v-btn>
                </v-card-actions>
            </v-card>
        </v-navigation-drawer>
    </div>
